...
<DefaultEvent("Scroll"), DefaultProperty("BarInnerColor")> _
...
#Region " Properties"
    ...
    Public Property ThumbCustomShape() As GraphicsPath
        ...
        Set(value As GraphicsPath)
            ...
            m_thumbSize = CType(If(barOrientation = Orientation.Horizontal, value.GetBounds().Width, value.GetBounds().Height), Integer) + 1
            Invalidate()
        End Set
    End Property
    ...
    Public Property Orientation() As Orientation
        ...
        Set(value As Orientation)
            If barOrientation <> value Then
                ...
                Width = Height
                Height = temp
                If Not (m_thumbCustomShape Is Nothing) Then
                    m_thumbSize = CType(If(barOrientation = Orientation.Horizontal, m_thumbCustomShape.GetBounds().Width, m_thumbCustomShape.GetBounds().Height), Integer) + 1
                End If
                Invalidate()
            End If
        End Set
...
#Region " Color schemas"
    ...
    Public Property ColorSchema() As ColorSchemas
        ...
        Set(value As ColorSchemas)
            m_colorSchema = value
            Dim sn As Byte = CType(value, Byte)
            m_thumbOuterColor = aColorSchema(sn, 0)
            m_thumbInnerColor = aColorSchema(sn, 1)
            m_thumbPenColor = aColorSchema(sn, 2)
            m_barOuterColor = aColorSchema(sn, 3)
            m_barInnerColor = aColorSchema(sn, 4)
            m_barPenColor = aColorSchema(sn, 5)
            m_elapsedOuterColor = aColorSchema(sn, 6)
            m_elapsedInnerColor = aColorSchema(sn, 7)
...
#Region " Paint"
    ...
    Protected Overrides Sub OnPaint(e As PaintEventArgs)
        If Not Enabled Then
            Dim desaturatedColors As Color() = DesaturateColors(m_thumbOuterColor, m_thumbInnerColor, m_thumbPenColor, m_barOuterColor, m_barInnerColor, m_barPenColor, _
                m_elapsedOuterColor, m_elapsedInnerColor)
        ...
        Else
            If m_mouseEffects AndAlso mouseInRegion Then
                Dim lightenedColors As Color() = LightenColors(m_thumbOuterColor, m_thumbInnerColor, m_thumbPenColor, m_barOuterColor, m_barInnerColor, m_barPenColor, _
                    m_elapsedOuterColor, m_elapsedInnerColor)
            ...
            Else
                DrawColorSlider(e, m_thumbOuterColor, m_thumbInnerColor, m_thumbPenColor, m_barOuterColor, m_barInnerColor, _
                    m_barPenColor, m_elapsedOuterColor, m_elapsedInnerColor)
            End If
        End If
    End Sub
    ...
    Private Sub DrawColorSlider(e As PaintEventArgs, thumbOuterColorPaint As Color, thumbInnerColorPaint As Color, thumbPenColorPaint As Color, barOuterColorPaint As Color, barInnerColorPaint As Color, _
        ...
        Try
            ...
            If barOrientation = Orientation.Horizontal Then
                Dim TrackX As Integer = (((trackerValue - barMinimum) * (ClientRectangle.Width - m_thumbSize)) / (barMaximum - barMinimum))
                m_thumbRect = New Rectangle(TrackX, 1, m_thumbSize - 1, ClientRectangle.Height - 3)
            Else
                Dim TrackY As Integer = (((trackerValue - barMinimum) * (ClientRectangle.Height - m_thumbSize)) / (barMaximum - barMinimum))
                m_thumbRect = New Rectangle(1, TrackY, ClientRectangle.Width - 3, m_thumbSize - 1)
            End If

            'adjust drawing rects
            barRect = ClientRectangle
            thumbHalfRect = m_thumbRect
            ...
            If barOrientation = Orientation.Horizontal Then
                ...
                elapsedRect = barRect
                elapsedRect.Width = m_thumbRect.Left + m_thumbSize / 2
            Else
                ...
                elapsedRect = barRect
                elapsedRect.Height = m_thumbRect.Top + m_thumbSize / 2
            End If
            'get thumb shape path 
            Dim thumbPath As GraphicsPath
            If m_thumbCustomShape Is Nothing Then
                thumbPath = CreateRoundRectPath(m_thumbRect, m_thumbRoundRectSize)
            Else
                thumbPath = m_thumbCustomShape
                Dim m As New Matrix()
                m.Translate(m_thumbRect.Left - thumbPath.GetBounds().Left, m_thumbRect.Top - thumbPath.GetBounds().Top)
            ...
            Using lgbBar As New LinearGradientBrush(barHalfRect, barOuterColorPaint, barInnerColorPaint, gradientOrientation)
                ...
                Using lgbElapsed As New LinearGradientBrush(barHalfRect, elapsedOuterColorPaint, elapsedInnerColorPaint, gradientOrientation)
                    ...
                    If Capture AndAlso m_drawSemitransparentThumb Then
            ...
            Using lgbThumb As New LinearGradientBrush(thumbHalfRect, newthumbOuterColorPaint, newthumbInnerColorPaint, gradientOrientation)
                ...
                If m_mouseEffects AndAlso (Capture OrElse mouseInThumbRegion) Then
            ...
            If Focused And m_drawFocusRectangle Then
                Using p As New Pen(Color.FromArgb(200, barPenColorPaint))
                    p.DashStyle = DashStyle.Dot
                    Dim r As Rectangle = ClientRectangle
                    r.Width -= 2
                    System.Math.Max(System.Threading.Interlocked.Decrement(r.Height), r.Height + 1)
                    System.Math.Max(System.Threading.Interlocked.Increment(r.X), r.X - 1)
                    'ControlPaint.DrawFocusRectangle(e.Graphics, r);                        
                    Using gpBorder As GraphicsPath = CreateRoundRectPath(r, m_borderRoundRectSize)
                        e.Graphics.SmoothingMode = SmoothingMode.AntiAlias
...
#Region " Overided events"
    ...
    Protected Overrides Sub OnMouseMove(e As MouseEventArgs)
        ...
        mouseInThumbRegion = IsPointInRect(e.Location, m_thumbRect)
        If Capture And e.Button = MouseButtons.Left Then
            ...
            Dim margin As Integer = m_thumbSize >> 1
            p -= margin
            Dim coef As Single = CType(barMaximum - barMinimum, Single) / CType((If(barOrientation = Orientation.Horizontal, ClientSize.Width, ClientSize.Height)) - 2 * margin, Single)
            trackerValue = CType(p * coef + barMinimum, Integer)
    ...
    Protected Overrides Sub OnMouseWheel(e As MouseEventArgs)
        ...
        Dim v As Integer = e.Delta / 120 * (barMaximum - barMinimum) / m_mouseWheelBarPartitions
    ...
    Protected Overrides Sub OnKeyUp(e As KeyEventArgs)
        ...
        Select Case e.KeyCode
            Case Keys.Down, Keys.Left
                SetProperValue(Value - CType(m_smallChange, Integer))
            ...
            Case Keys.[End]
                ...
                Exit Select
            Case Keys.PageDown
                SetProperValue(Value - CType(m_largeChange, Integer))
...
#Region " Help routines"
    ...
    Public Shared Function DesaturateColors(ParamArray colorsToDesaturate As Color()) As Color()
        ...
        Dim i As Integer = 0
        While i < colorsToDesaturate.Length
            'use NTSC weighted avarage
            Dim gray As Integer = CType(colorsToDesaturate(i).R * 0.3 + colorsToDesaturate(i).G * 0.6 + colorsToDesaturate(i).B * 0.1, Integer)
